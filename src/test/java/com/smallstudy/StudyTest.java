...
public class StudyTest {
    ...
    @DisplayName("스터디 생성 화면 GET")
    @Test
    void 스터디_생성_화면_기본_로컬캐시() throws Exception {
        MockHttpSession session = new MockHttpSession();
        SetSecuritySession(new Member("test123@google.com", "admin", "password!"));
        mockMvc.perform(get("/study").session(session))
                .andExpect(status().isOk())
                .andExpect(view().name("smallstudy/study/study"))
                .andExpect(model().attributeExists("categories", "regions", "selectedCategories", "selectedRegions", "form"))
                .andExpect(model().attribute("categories", MyLocalCache.categoryItems))
                .andExpect(model().attribute("regions", MyLocalCache.regions))
                .andExpect(model().attribute("selectedCategories", List.of()))
                .andExpect(model().attribute("selectedRegions", List.of()));
    }

    @DisplayName("스터디 생성 화면, 맴버 관심 지역 Get")
    @Test
    void 스터디_생성_화면_맴버_관심_지역() throws Exception {

        Member save = getMemberWithMemberSelectedRegionsAndCategoryItem();
        SetSecuritySession(save);

        List<CategoryItemDTO> categoryItems = categoryItemService.findCategoryItemDTOsByMemberId(save.getId());
        List<RegionDTO> regions = interestRegionService.getMemberSelectedRegions(save.getId());

        MvcResult mvcResult = mockMvc.perform(get("/study"))
                .andExpect(status().isOk())
                .andExpect(view().name("smallstudy/study/study"))
                .andExpect(model().attributeExists("categories", "regions", "selectedCategories", "selectedRegions", "form"))
                .andExpect(model().attribute("categories", categoryItems))
                .andExpect(model().attribute("regions", regions))
                .andReturn();

        Object c1 = getModelData("selectedCategories", mvcResult);
        Object r1 = getModelData("selectedRegions", mvcResult);

        List<CategoryItemDTO> selectedCategory = (List<CategoryItemDTO>) c1;
        List<RegionDTO> selectedRegion = (List<RegionDTO>) r1;
        List<Long> t1 = categoryItems.stream().map(CategoryItemDTO::getCode)
                .sorted(Comparator.comparingLong(Long::longValue))
                .toList();
        List<Long> t2 = selectedCategory.stream()
                .map(CategoryItemDTO::getCode)
                .sorted(Comparator.comparingLong(Long::longValue))
                .toList();
        List<Long> m1 = regions.stream()
                .map(RegionDTO::getCode)
                .sorted(Comparator.comparingLong(Long::longValue))
                .toList();
        List<Long> m2 = selectedRegion.stream()
                .map(RegionDTO::getCode)
                .sorted(Comparator.comparingLong(Long::longValue))
                .toList();
    ...
    private Member getMemberWithMemberSelectedRegionsAndCategoryItem() {
        Member member = new Member("test6@google.com", "admin1232", "password!");
    ...
    private  void SetSecuritySession(Member save) {
